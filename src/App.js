...
function App() {
	...
	useEffect(() => {
		...
		fetch(`https://api.github.com/search/repositories?q=created:>${sinceStr}&sort=stars&order=desc&per_page=25`, {
			headers: auth,
		})
			...
			.then(function(data) {
				const repoUrlArray = data.items.map((item) => `/${item.full_name}`);
